export default function Avatar({ userId, username, online,profile }) {
    // Define an array of background colors for the avatar
    const colors = [
      "bg-teal-200",
      "bg-red-200",
      "bg-green-200",
      "bg-purple-200",
      "bg-blue-200",
      "bg-yellow-200",
      "bg-orange-200",
      "bg-pink-200",
      "bg-fuchsia-200",
      "bg-rose-200",
    ];
  
    // Convert the userId to base 10 and calculate the color index
    const userIdBase10 = parseInt(userId.substring(10), 16);
    const colorIndex = userIdBase10 % colors.length;
  
    // Select the color from the colors array based on the color index
    const color = colors[colorIndex];
  
    return (
      <div className={(!profile?"w-8 h-8":"w-10 h-10") + " relative rounded-full flex items-center " + color}>
        {/* Display the first character of the username */}
        <div className="text-center w-full opacity-70">{username[0]}</div>
  
        {/* Render a green dot if the user is online */}
        {online && (
          <div className="absolute w-3 h-3 bg-green-400 bottom-0 right-0 rounded-full border border-white"></div>
        )}
  
        {/* Render a gray dot if the user is offline */}
        {!online && (
          <div className="absolute w-3 h-3 bg-gray-400 bottom-0 right-0 rounded-full border border-white"></div>
        )}
      </div>
    );
  }
  